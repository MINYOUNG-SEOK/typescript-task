...
function calculateShippingCost(region, costs) {
    if (!(region in costs)) {
        throw new Error("\uC9C0\uC6D0\uD558\uC9C0 \uC54A\uB294 \uCF54\uB4DC: ".concat(region));
    }
    return costs[region];
}
// 테스트 코드
...
function calculateAverageScore(scores) {
    var values = Object.keys(scores).map(function (key) { return scores[key]; });
    var total = values.reduce(function (sum, score) { return sum + score; }, 0);
...
function updateProductPrice(prices, product, newPrice) {
    ...
    if (!(product in prices)) {
        throw new Error("\uC81C\uD488\uC774 \uC874\uC7AC\uD558\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4: ".concat(product));
    }
